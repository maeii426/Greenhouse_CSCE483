...
CONFIGURATION_PORT = 0 # Greenhouse port
...
def main():
	...
	global CONFIGURATION_PORT
	global CONFIG_PATH
	try:
		config = ConfigParser.ConfigParser()
		config.readfp(open(CONFIG_PATH))
		GREENHOUSE_IP = config.get('Properties', 'GREENHOUSE_IP')
		CONFIGURATION_PORT = config.get('Properties', 'CONFIGURATION_PORT')
		sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		server_address = (GREENHOUSE_IP, int(GREENHOUSE_PORT))
